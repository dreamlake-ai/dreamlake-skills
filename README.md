...
| [`dreamlake-notes`](./dreamlake-notes/SKILL.md) | Read and edit collaborative notes — replace text by name rather than line number, read part of a long note, grep across every note for where a phrase is, attach files and get a link that renders them |
| [`dreamlake-cli`](./dreamlake-cli/SKILL.md) | The whole `dreamlake` CLI reference, generated from its docs. Install this when the binary came from `dl.dreamlake.ai` rather than npm — that build cannot unpack its own bundled copy |
