---
name: dreamlake-viz
description: @dreamlake/viz visualizes robot-learning datasets in the browser: one `.dreamrc` file at a dataset root renders every episode — LeRobot / zarr / MCAP / plain folders, cameras, depth, point clouds, time series, annotations, and 3D reconstructions, all read in place over HTTP. Use when answering questions about DreamLake (Quick start, Start here, EpisodeTimeline, Prepare your data, Overview, Write the .dreamrc, View components, Reference, Templates, EpisodeVideoStack, The architecture, Schema, Gallery, EpisodeFrameStack, Media overlays, EpisodeRecon3d, EpisodeLineChart, FilePreview, Storage, Views, Adapters, Loaders, Views, Concept, LLM-Readable Docs).
---
# DreamLake
...
- `reference/dataset-viz-start.md` — Start here: From data to a rendered dataset in five steps: see what can be visualized, pick your data route, drop in a minimal .dreamrc, read the inventory, bind views, and iterate live in the gallery playground.
- `reference/dataset-viz-requirements.md` — Prepare your data: Three routes into the viewer — an existing LeRobot/zarr/MCAP dataset (nothing to do), raw recordings arranged as folders (a concrete recipe), or a container we cannot read yet — plus the shape every payload needs, where annotations go, and the checks before you ship.
- `reference/dataset-viz-spec.md` — Write the .dreamrc: The whole config language with every key, its default and an example: the dataset block per format, view bindings and the as: rule, sizing and layout, labels and annotations, storage, and the validation loop.
- `reference/dataset-viz-views.md` — View components: Every view, live: a minimal .dreamrc, its render, and a table of every option the view takes — slots, keys, defaults, and when you need as:.
...
- `reference/dataset-viz-overview.md` — The architecture: Why the system is shaped the way it is: pre-built view components each declare an input contract; the contracts are a small closed set of in-memory payload kinds; the bytes on disk stay in existing formats, normalized by adapters; and the .dreamrc is where meaning is stated. Ends under the hood — the code-level pipeline, adapter discipline, registries and laziness rules — for people changing the library.
- `reference/dataset-viz-gallery.md` — Gallery: A playground of complete .dreamrc files over real public datasets — LeRobot v2/v3 (video, depth, point clouds), UMI zip and zarr stores, MCAP logs, raw folders with annotations and 3D reconstruction — switch between them, edit the file, and watch the render follow.
